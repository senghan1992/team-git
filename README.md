...
| 문제 | 이 앱의 답 |
| --- | --- |
| 팀원이 push했는지 병합 관리자가 모른다 | push는 작업 공유, **병합 요청**은 승인 신호로 분리했다. 요청이 오면 **그 브랜치의 병합 관리자에게만** 알림이 가고, 홈 카드에 "N건 병합 승인"으로 남는다. 관리자는 승인 대기열에서 요청을 검토하고 **병합하기** 한 번으로 승인한다. |
| AI 에이전트를 쓰기 시작하니 어디가 어떻게 고쳐지는지 안 보인다 | 병합 탭의 **변경 지도** — 파일 기준으로 뒤집어, 여러 브랜치가 손댄 파일을 맨 위에 세우고, 충돌이 덜 나는 **병합 권장 순서**까지 제안한다. |
| 병합하다 충돌이 나면 거기서 멈춘다 | **AI 자동 병합** — OpenAI 호환 엔드포인트면 무엇이든 연결 가능(회사 LLM·pi·Ollama 등, 설정에서 **연결 테스트**로 확인). 지침(프롬프트)을 미리 저장해 두면 충돌이 난 순간 알아서 고치고 병합 커밋까지 만든다. 원본은 항상 백업되고, 충돌 표시가 남은 파일은 절대 커밋되지 않으며, AI가 못 고친 파일을 통째로 한쪽만 남기지도 않는다(팀원의 커밋이 조용히 사라지지 않게). AI가 고친 병합은 관리자가 결과를 확인한 뒤에야 push된다. |
...
역할은 두 가지뿐입니다: **병합 관리자**(한 브랜치에 여러 명 지정 가능, 팀원들의
브랜치를 main으로 모으는 사람)와 **일반 팀원**. 아래 순서 그대로 따라 하면 됩니다. 더 깊은 내용은
[docs/WORKFLOW.md](docs/WORKFLOW.md)에 다 있습니다.

> 앱 안에도 같은 가이드가 들어 있습니다 — 사이드바 맨 아래 **내 이름을 누르면**
> **이용 가이드** 탭이 열립니다. 팀의 한 바퀴(등록 → 작업 → 병합 요청 → 승인 →
> 동기화)와 내 역할이 화면과 함께 나오니, README를 안 읽은 팀원에게 여기만
> 알려 줘도 됩니다.
...
### 0-2. 서버 운영자 — 관리자 대시보드 (브라우저)

**앱 설치 없이** 브라우저로 서버 주소를 열면 관리 대시보드가 열립니다:
`http://<서버IP>:48111` (방법 B 로 직접 띄웠다면 `http://<서버IP>:8000`).
관리자 계정으로 로그인하면 앱의 관리자 화면과 같은 내용을 웹에서 볼 수
있습니다 — 사용자·세션·프로젝트 현황, 최근 활동 피드, 계정 정지/강제
로그아웃/멤버 제거/프로젝트 삭제. 20초마다 조용히 새로 고칩니다.

앱 안의 관리자 화면(사이드바 자물쇠)과 같은 API 를 쓰므로, 어느 쪽에서
해도 결과는 같습니다. 웹 대시보드는 앱이 없는 컴퓨터(예: 서버 관리용
태블릿·휴대폰)에서 볼 때 유용합니다.

관리자 임명 전에는 어떤 계정으로도 데이터를 볼 수 없습니다 — 먼저 아래의
`adminctl` 로 관리자를 정하세요.

- **관리자 임명** — 관리자는 가입 순서가 아니라 서버 운영자가 직접 정합니다.
  서버 컴퓨터에서:
...
- **추적** — 개요 탭에서 사용자·세션·기기·프로젝트 수와 24시간/주간 활동량,
  ...
  활동량, 활동 탭에서 누가 어느 저장소에 무엇을 했는지 시간순 피드를 봅니다.
  20초마다 조용히 새로 고칩니다.
- **제어** — 계정 정지(세션 즉시 삭제, 재로그인도 차단)·정지 해제, 강제
  로그아웃, 프로젝트에서 멤버 제거, 프로젝트 삭제(멤버·이벤트 기록까지 —
  저장소 파일과 커밋은 건드리지 않습니다). 파괴적인 동작은 확인 대화상자를
  거치고, 자기 계정은 정지할 수 없습니다.
- 일반 사용자에게는 관리자 화면이 아예 보이지 않고, 서버도 403으로 이중으로
  막습니다.

### 1. 관리자가 한 번만 — 저장소 등록 + 팀 규칙

**+ 저장소 추가**로 프로젝트 폴더를 등록하고, 저장소의 **설정** 탭에서
가입자 검색으로 **구성원**을 넣고 **브랜치별 병합 관리자**를 지정합니다
(관리자는 여러 명 지정 가능). 이 규칙은 `.gpconfig`
...
### 3. 관리자의 하루 — 병합 요청 받고 승인

팀원이 push 후 **병합 요청 보내기**를 누르면 관리자가 **어느 화면에 있든**
오른쪽 아래에 "○○ 브랜치 병합 요청 도착" 알림이 뜨고(**검토하기** 버튼으로
바로 이동), 사이드바 **알림** 배지에 읽지 않은 수가 남습니다. 홈 카드에는
**"N건 병합 승인"**으로 쌓입니다. push만 한 브랜치는 대기열에 오르지 않습니다 —
병합할 것이 생겼다는 신호는 요청 하나입니다. 알림 탭에서는 카드별 **읽음 표시**와
**모두 읽음**으로 정리합니다.
...
- **저장소 → 작업** — 브랜치 전환, 새 브랜치 만들기, 상태 표(diff 미리보기,
  파일별 스테이징), 커밋 / 푸시 / 풀 / 스태시, 동기화, **병합 요청**(푸시 후
  관리자 승인 요청 — 갱신·취소 지원).
- **저장소 → 병합** — 상단 **최근 7일 병합 흐름**(어떤 브랜치가 언제 작업돼
  언제 main에 합쳐졌는지 시간축으로, 미병합 브랜치는 점선; 7/14/30일), 변경 지도,
  **병합 요청 승인 대기열**(요청 검토 → 병합하기/요청 거절), 블록 단위 충돌 해결(나의 것 / 상대 것 /
...
| HTTPS 푸시가 아이디·비밀번호를 정확히 입력해도 "로그인 실패"로 거부된다 | 옛 버전은 임시 askpass 스크립트로 인증을 넣었는데 Windows Git 이 이 스크립트 실행을 신뢰성 있게 처리하지 못해 전부 거부됐습니다. v0.1.4부터는 `http.extraheader`(Basic) 로 직접 주입하므로 모든 플랫폼·특수문자 비밀번호에서 동일하게 동작합니다. |
