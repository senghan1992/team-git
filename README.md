...
| AI 에이전트를 쓰기 시작하니 어디가 어떻게 고쳐지는지 안 보인다 | 병합 탭의 **변경 지도** — 파일 기준으로 뒤집어, 여러 브랜치가 손댄 파일을 맨 위에 세운다. |
| 병합하다 충돌이 나면 거기서 멈춘다 | **AI 자동 병합** — 지침(프롬프트)을 미리 저장해 두면 충돌이 난 순간 알아서 고치고 병합 커밋까지 만든다. 원본은 항상 백업되고, 충돌 표시가 남은 파일은 절대 커밋되지 않으며, AI가 못 고친 파일을 통째로 한쪽만 남기지도 않는다(팀원의 커밋이 조용히 사라지지 않게). |
