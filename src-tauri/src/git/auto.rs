...
pub struct AutoResolveOptions {
    /// Side used for binary / oversized files (default: theirs).
...
fn fallback_side(detail: &ConflictDetail) -> SideChoice {
    // If the incoming branch never touched the file, our version is the
    // deliverable — keep it. Otherwise take theirs (the teammate's work).
    if detail.base.as_deref() == Some(detail.theirs.as_str()) {
        SideChoice::Ours
    } else {
        SideChoice::Theirs
    }
...
mod tests {
    ...
    #[test]
    fn fallback_prefers_ours_when_branch_did_not_touch_file() {
        ...
        assert_eq!(fallback_side(&d), SideChoice::Ours);

        let d2 = ConflictDetail {
            path: "a.txt".into(),
            is_binary: false,
            too_large: false,
            base: Some("shared\n".into()),
        ...
        assert_eq!(fallback_side(&d2), SideChoice::Theirs);
    }
