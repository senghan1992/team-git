...
Git Companion is a lightweight desktop companion for git teams working one project through per-person branches. It closes a single loop: each member cuts their own branch, commits, and pushes to their own remote branch (**push = sharing work, not a merge ask**); the member then sends an explicit **merge request** (stored as a shared git ref `refs/gc-mr/<base>/<branch>`) that pins the requested tip; the designated **merge manager** reviews the approval queue (요청자·제목·파일 diff·커밋), approves (병합하기 — merges exactly the pinned commit) or rejects, pushes the merge branch, and every member is then notified and syncs the merged code back into their branch. Around that loop it keeps repository state in view (branch, ahead/behind, working tree), suggests the one next action per repository, shows a file-centric **change map** of what every requested branch is touching, and repairs merge conflicts with a pre-configured AI resolver.
...
- Flow: register repo → set `.gpconfig` (merge branch + merge manager) → member cuts a branch, works, commits, pushes, **sends a merge request** → manager is notified, reviews the approval queue, approves (merge, AI repairs conflicts) and pushes → members are notified and sync. Stash for temporary parking; team inbox for push + merge-request events; settings for AI auto-merge, SSH, push credentials, and account. Documented end to end in `docs/WORKFLOW.md`.
...
- Merge tab: **승인 대기열** — only branches with an explicit merge request appear (requests are git refs `refs/gc-mr/<base>/<branch>`, shared via origin, fetched with an extra refspec; the requested tip is pinned so what was reviewed is what merges). Each request card shows requester, requested time, title, ahead/behind, changed files (click = diff), collapsible commits; actions are **병합하기 (승인)** and **요청 거절** (manager-gated). **변경 지도** maps requests to files (files touched by two or more requests pinned on top with a warning, rest collapsed), 20 s auto-detect, one merge-manager badge for the base branch, block-level conflict resolution (ours/theirs/manual, per-block AI suggestion, undo), one-click AI auto-merge, pre-resolution backup restore, post-merge push banner. Merges and pushes to a managed branch are locked for non-managers.
- Work tab (member): after a successful push a **병합 요청** card offers **병합 요청 보내기** (title modal, default = last commit subject), then **요청됨 · 관리자 승인 대기** with **요청 갱신** (re-pin to latest pushed tip) and **요청 취소** (withdraw). Requests are only allowed on pushed commits; already-merged or no-change branches are rejected.
