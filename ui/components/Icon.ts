// Inline SVG icon set. Lucide (ISC-licensed) path data — see
// https://github.com/lucide-icons/lucide/tree/main/icons. We re-use the
// canonical 24x24 stroke shapes so they render consistently with the rest
// of the lucide family. `icon()` returns a fresh SVG element so callers
// can place it inline (inside buttons, badges, banners, etc.).
...
export function spinner(size = 16): SVGSVGElement {
  const svg = icon("loader", size);
  svg.classList.add("gc-spin");
  return svg;
}
